...
const ColActionTwo = ({
	...
	const { currentAllowance, handleApproval: handleLPTokenApproval, transactionStatus: approvalTransactionStatus } = useTokenApproval(CONTRACTS_TYPE.LP_TOKEN, farmType);
			...
			return isEnabled ? (
				totalUserStaked ? (
					<>
						<Button
							type='button'
							text={isTransactionInProgress && transactionType === FARMS_ACTIONS.UNSTAKE ? 'Unstaking...' : 'Unstake'}
						...
						<Button
							type='button'
							text={isTransactionInProgress && transactionType === FARMS_ACTIONS.STAKE ? 'Staking...' : 'Stake'}
				...
				<Button
					type='button'
					text={approvalTransactionStatus.status === TRANSACTION_STATUS.SUBMITTED ? 'Enabling...' : 'Enable'}
					value={FARMS_ACTIONS.APPROVAL}
					classes={`w-100 ${media === 'mobile' ? 'mt-2' : ''}`}
					onClick={isActive ? handleApproval : voidFunc}
					green
					disabled={!isActive || approvalTransactionStatus.status === TRANSACTION_STATUS.SUBMITTED}
				/>
			);
		} 
	};
	...
	const mobileHTML = (
		<div className='farms__box__action'>
			...
			>
				...
				{account ? (
					<div className='txt-right'>
						<span className='font-weight-700'>{balanceFormatter.format(totalUserStaked)}</span>
					</div>
				) : (
					'-'
				)}
	...
	const desktopHTML = (
		<div className={`farms__box__action ${isOpened ? 'farms__box__action__middle-col' : ''}`}>
			<div className='farms__box__action__info'>
				...
				{account ? (
					<div className='txt-right'>
						<span className='font-weight-700'>{!isActive ? "-" : balanceFormatter.format(totalUserStaked)}</span>
					</div>
				) : (
					<div className='txt-right'>
			...
			{isOpened && <div className='d-flex w-100'>{renderButton('desktop')}</div>}
		</div>
	);
