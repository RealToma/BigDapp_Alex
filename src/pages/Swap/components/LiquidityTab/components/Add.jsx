	...
	const invokeAddLiquidity = async () => {
		toggleConfirmModal();
		  try {
			...
			if (selectedValue.tokenFrom.name === 'BUSD') {
				inputAmountWithSlippage = parseUnits((swap?.inputFrom * 0.98)?.toString(), 18);
			} else if (selectedValue.tokenFrom.name === 'USDT' ){
				if (chainId === 1) {
					inputAmountWithSlippage = parseUnits((swap?.inputFrom * 0.98)?.toFixed(6), 6);
				} else {
					inputAmountWithSlippage = parseUnits((swap?.inputFrom * 0.98)?.toString(), 18);
				}
			} else {
			...
			if (selectedValue.tokenTo.name === 'BUSD') {
				outputAmountWithSlippage = parseUnits((swap?.inputTo * 0.98)?.toString(), 18);
			} else if (selectedValue.tokenTo.name === 'USDT' ){
				if (chainId === 1) {
					outputAmountWithSlippage = parseUnits((swap?.inputTo * 0.98)?.toFixed(6), 6);
				} else {
					outputAmountWithSlippage = parseUnits((swap?.inputTo * 0.98)?.toString(), 18);
				}
			} else {
